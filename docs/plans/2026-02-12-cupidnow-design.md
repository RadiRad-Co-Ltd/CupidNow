...
> **狀態**：已完成 MVP 並上線部署（2026-02-12）
> - 前端：https://cupidnow.netlify.app
> - 後端：https://cupidnow-api.onrender.com
...
| 圖表 | 自製 CSS 元件（不依賴外部圖表庫） |
| 圖示 | Lucide React |
| 截圖/分享 | html-to-image + Web Share API |
| 後端 | Python 3.12 + FastAPI |
| 數據分析 | pandas, jieba (中文斷詞) |
| AI 分析 | Claude API (Anthropic) |
| 部署 | Netlify (前端) + Render Docker (後端) |
...
│  Tailwind v4    │    明文傳輸/結果     │  jieba / pandas   │
│  Lucide React   │                    │                    │
└─────────────────┘                    └────────┬─────────┘
   Netlify CDN                                  │ 摘要抽樣
                                                ▼
...
> **變更記錄**：原設計包含前端 AES-256-GCM 加密，後經安全審查移除（commit `2aebe73`），改為 HTTPS 明文傳輸 + 伺服器端安全措施。

### 安全機制

- **HTTPS 傳輸**：所有 API 通訊透過 HTTPS 加密
- **速率限制**：每 IP 每 60 秒最多 10 次請求
- **檔案大小限制**：上傳檔案最大 10MB
- **記憶體處理**：後端僅在記憶體中處理，絕不寫入磁碟
- **即時銷毀**：分析完成後立即清除所有原始數據，僅回傳統計結果
- **CORS 白名單**：僅允許指定來源（localhost + 生產域名）
- **無帳號系統**：不收集任何使用者個人資訊
- **金句例外**：AI 挑選的金句會包含原始文字，但僅在使用者瀏覽器端顯示
...
│   ├── file: LINE 匯出的 .txt 聊天記錄
│   └── skip_ai: (選用) 跳過 AI 分析
├── Response: JSON 分析結果
└── 預估處理時間: 5-15 秒（不含 AI）/ 15-30 秒（含 AI）

GET  /api/health
└── 健康檢查
```

> **變更**：原設計包含 AES 加密金鑰傳輸與 SSE 進度推送，實際實作簡化為明文上傳 + 同步 JSON 回應。

### 後端處理 Pipeline

```
1. 接收 .txt 檔案 → UTF-8 解碼（記憶體中）
...
14. ✅ 文字雲（詞頻大小排列，非愛心形狀）
15. ✅ 專屬用語偵測
16. ✅ 分享卡片生成（html-to-image + Web Share API）
17. ⬜ ~~SSE 進度推送~~ → 使用簡易進度條替代
18. ✅ 手機 RWD 直式排版
19. ✅ Lucide React 圖示替換（取代 emoji）
20. ✅ Netlify + Render 部署與 CI/CD
